...
class LoRALayer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, rank: int, alpha: float) -> None:
        ...
        self.B = nn.Parameter(torch.zeros(rank, out_dim)) # B is initialized to zero as to 
        # not affect the output initially, the weights will be learned during training
        self.alpha = alpha # Scaling factor for LoRA
        #when we save the adapter weights, we will save A and B, (new parameters) for each layer
...
class LinearWithLoRA(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.linear(x) + self.lora(x) # Adding the LoRA output to the original linear output
    ...
    for name, child in list(module.named_children()):
        if isinstance(child, nn.Linear):
            ...
            _replace_linear_layers_with_lora_layers(
                child, lora_config) #we recursively replace linear layers in the module
